...
from sensor.entity.config_entity import DataValidataionConfig
...
class DataValidation:
    def __init__(self,data_validation_config:DataValidataionConfig,
    ...
    def drop_missing_values_columns(self,df:pd.DataFrame,report_key_name:str)->Optional[pd.DataFrame]:
        try:
            ...
            logging.info(f"selecting column name which conatins null above to {threshold}")
    ...
    def is_required_columns_exists(self,df:pd.DataFrame,report_key_name:str)->bool:
        try:
            ...
            logging.info("Required columns:{required_columns}")
    ...
    def drop_columns(self,df:pd.DataFrame)->pd.DataFrame:
        try:
            ...
            logging.info(f"Dropping columns based on schema provoded:{drop_columns}")
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            train_df=self.drop_missing_values_columns(df=train_df,report_key_name="train_missing_values_ccolumns")
            test_df=self.drop_missing_values_columns(df=test_df,report_key_name="test_missing_values_ccolumns")
            ...
            is_exist=self.is_required_columns_exists(df=train_df,report_key_name="train_required_column")
            if not is_exist:
                raise Exception("Required columns are not available in train df")
            
            is_exist=self.is_required_columns_exists(df=test_df,report_key_name="test_required_column")
            if not is_exist:
                raise Exception("Required columns are not available in test df")
            
            if len(train_df.columns)!=len(test_df.columns):
                raise Exception(f"Train and test df doest not have equal columns")
            ...
            data_validation_artifact=DataValidationArtifact(report_file_path=self.data_validation_config.report_file_name, train_file_path=self.data_validation_config.valid_train_file_path, 
